    ...
    try {
        ...
        if (!errors.isEmpty()) {
            return res.status(400).json({
                errors: errors.array(),
                message: 'Некорректно введенные данные во время регистрации'
            })
        }
        ...
        if (candidate) {
            return res.status(400).json({message: 'Email адрес уже существует'})
        }
    ...
    async (req, res) => {
        try {
            ...
            if (!errors.isEmpty()) {
                return res.status(400).json({
                    errors: errors.array(),
                    message: 'Некорректно введеные данные при входе'
                })
            }
